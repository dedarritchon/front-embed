...
import type { GoogleMapsSearchProps } from '../types/googleMaps';
...
export const GoogleMapsSearch: React.FC<GoogleMapsSearchProps> = ({
  apiKey,
  onLocationSelect,
  ...
  const {
    ...
    handleInputBlur,
    handleSearch,
  } = useGoogleMapsSearch(apiKey, onLocationSelect);

  // Show a message if Google Maps API is not available
  if (!apiKey) {
    return (
      <SearchContainer>
        <ErrorMessage>
          <Paragraph color="#6c757d">
            Google Maps API key not configured. Autocomplete unavailable.
          </Paragraph>
        </ErrorMessage>
      </SearchContainer>
    );
